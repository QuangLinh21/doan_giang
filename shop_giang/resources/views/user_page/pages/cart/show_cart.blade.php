                    ...
                    <table class="table axil-product-table axil-cart-table mb--40">
                        ...
                        <tbody>
                            ...
                            <tr>
                                ...
                                <td class="product-thumbnail"><img src="{{ asset($item->options->images) }}" alt="Digital Product"></td>
                                <td class="product-title"><a href="single-product.html">{{$item->name}}</a></td>
                                <td class="product-price" data-title="Price">{{ number_format($item->price, 0, ',', '.') }}<span class="currency-symbol">VNĐ</span></td>
                                <td class="product-quantity" data-title="Số lượng">
                                    <div class="" data-title="Số lượng">
                                       <form action="{{URL::to('update-cart-qty')}}" method="post">
                                        @csrf
                                            <input type="number" name="cart_qty" class="quantity-input" value="{{$item->qty}}">
                                            <input type="hidden" value="{{$item->rowId}}" name="rowId_cart">
                                            <td class="product-remove"><input type="submit" value="Cập nhật" name="update_qty"></a></td>
                                       </form>
                                    </div>
                                </td>
...
;                                    ?>
                                    ...
                                    <td class="product-remove"><a href="{{URL::to('delete-cart-item/'.$item->rowId)}}" class="remove-wishlist"  onclick="return confirm('Bạn có muốn xóa danh mục này không?')"><i class="fal fa-times"></i></a></td>
                ...
                <div class="row">
                    <div class="col-xl-5 col-lg-7 offset-xl-7 offset-lg-5">
                        <div class="axil-order-summery mt--80">
                            ...
                            <a href="{{URL::to('login_checkout')}}" class="axil-btn btn-bg-primary checkout-btn">Thanh toán</a>
                        </div>
                    </div>
                </div>
            </div>
